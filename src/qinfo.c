...
int QInfo_create(QInfo *info) {
  ...
  *info = (QInfo_impl_t *)malloc(sizeof(QInfo_impl_t));
  ...
  (*info)->value_space = (QInfo_value_space_t *)malloc(
      sizeof(QInfo_value_space_t) * (*info)->topic_space_size);
...
int QInfo_duplicate(QInfo info_in, QInfo *info_out) {
  ...
  *info_out = (QInfo_impl_t *)malloc(sizeof(QInfo_impl_t));
  ...
  (*info_out)->value_space = (QInfo_value_space_t *)malloc(
      sizeof(QInfo_value_space_t) * (*info_out)->topic_space_size);
...
int QInfo_topic_add(QInfo info, char *topicname, QInfo_type type,
                    QInfo_topic *topic) {
  int i, old_size;
  ...
  if (info->topic_space_size == info->topic_space_occupied) {
    ...
    info->topic_space_size += QINFO_INTERNAL_SPACEGRANULARITY;
    info->value_space = (QInfo_value_space_t *)realloc(
        info->value_space,
        sizeof(QInfo_value_space_t) * (info->topic_space_size));
    if (info->value_space == NULL)
      return QINFO_ERROR_OUTOFMEM;
...
int QInfo_topic_getnext(QInfo info, QInfo_iterator *iter, QInfo_topic *topic,
                        char **topicname) {
  int i = 0;
